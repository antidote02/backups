...
`自动开始下载下列类型的文件：`
      ```
      3GP 7Z AAC ACE AIF APK ARJ ASF AVI BAT BIN BZ2 EXE GZ GZIP IMG ISO JAR LOG LZH M4A M4V MKV MOV MP3 MP4 MPA MPE MPEG MPG MSI MSU NUPKG OGG OGV PDF PLJ PPS PPT QT R0* R1* RA RAR RM RMVB SEA SIT SITX TAR TIF TIFF TORRENT WAV WMA WMV Z ZIP
      ```
    * `保存至`  
`另存为`  
